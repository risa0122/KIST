...
# %% start the cluster
    ...
    if prepro == True:
        for i in range(10):
            ...
            b = nd2_np[i]   # 304,304
            y = filters.rank.equalize(b, selem=morphology.disk(30))    #  `image` must be a 2-dimensional array
            y = exposure.equalize_adapthist(y, clip_limit=10)
            y = filters.median(y)
            vv.append(y)
    vv = np.array(vv)
    #io.imshow('/Users/taco-lab/PycharmProjects/caiman2/Slicing_nd2/0.tiff')
    #print('##vv##', np.array(vv).shape)   # 10,304,304
    #print(vv)
        #vv = [np.array(vv).reshape(1,1797)]    #got error
    ...
    for i in range(10):
        ...
        cv2.imwrite('/Users/taco-lab/PycharmProjects/caiman2/Slicing_nd2/{}.tiff'.format(i), fnames_test)



    fnames = [sorted(glob.glob('/Users/taco-lab/PycharmProjects/caiman2/Slicing_nd2/*.tiff'))]
    print(np.array(fnames).shape)   # (1,1797)

    filename_reorder = fnames
    '''
    path_dir = '/Users/taco-lab/PycharmProjects/caiman2/Slicing_nd2/'
    ...
    fnames = [sorted(file for file in fnames if file.endswith(".tiff"))]  #(1797,)
    print('##111fnames.shape###', np.array(fnames).shape)
    #fnames = np.expand_dims(fnames,axis=0)

    fnames = np.delete(fnames,np.reshape(1,0))
    #fnames = np.transpose(fnames)  #(1797,)

    #fnames = [np.reshape(fnames,(-1,1))]    #(1797,1) 열벡

    #fnames = np.vectorize([fnames])


    print('##fnames.shape###',np.array(fnames).shape)
    #fnames = np.reshape(fnames,(1,))    # ValueError: cannot reshape array of size 1797 into shape (1,)



    filename_reorder = fnames
    print('파일네임리오더',np.array(filename_reorder))   # list[str]
'''

    fr = 10  # movie frame rate
    decay_time = 0.4  # length of a typical transient in seconds

    fname_new = cm.save_memmap(filename_reorder, base_name='memmap_', order='C', border_to_0=0, dview=dview)
    #print("file's name: ",fname_new)

    Yr, dims, T = cm.load_memmap(fname_new)
    #print('dimdimdimddim',dims)
    ...
    dims = np.shape(nd2_np[0,:,:])  # (304,304)
    #print(dims)
    ...
    cnm.fit(images)
    #print('이미지 형태2 : ', images.shape)  # 1797,304,304
    ...
    #img = np.reshape(np.array(images.mean(axis=1)), dims, order='F')
    #print('*****img*******',img)
    if display_images:
